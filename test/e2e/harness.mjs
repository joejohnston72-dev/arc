...
export async function serve(port = 4173) {
  const server = http.createServer((req, res) => {
    ...
    // The Supabase test double is swapped in by the server itself, not by a
    // Playwright route: with a route, Chromium could still parse the real module
    // for one of its importers, and its CDN import then failed the whole graph.
    const file = p === '/shared/supabase.js' ? STUB : p === '/sw.js' ? '' : path.join(ROOT, p);
...
export async function boot({ port = 4173, seed = null } = {}) {
  ...
  // The Supabase stub and the sw.js 404 are handled by serve() above. Only
  // outbound traffic is cut here. Regex, not glob: glob patterns failed to match
  // in this Playwright/Chromium setup and let requests through.
