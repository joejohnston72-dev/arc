...
// Headless Chromium never shows a keyboard, so drive what fitCoach() sets — the
// visible strip (--vv-top/--vv-h) and body.coach-kb — and check the layout it
// feeds: the composer rides the keyboard and the tab bar steps aside.
const beforeKb = await page.$eval('.coach-input', e => Math.round(e.getBoundingClientRect().bottom));
await page.evaluate(() => {
  const root = document.documentElement;
  root.style.setProperty('--vv-top', '0px');
  root.style.setProperty('--vv-h', (innerHeight - 300) + 'px');
  document.body.classList.add('coach-kb');
});
await page.waitForTimeout(200);
const kb = await page.evaluate(() => ({
  bottom: Math.round(document.querySelector('.coach-input').getBoundingClientRect().bottom),
  limit: innerHeight - 300,
  tabs: getComputedStyle(document.querySelector('.tabs')).display,
}));
ok(kb.bottom <= kb.limit + 1, `a 300px keyboard keeps the composer visible (bottom ${kb.bottom}px, keyboard top ${kb.limit}px)`);
eq(kb.tabs, 'none', 'and the tab bar steps aside while typing');
await shot(page, 'X5-keyboard');
await page.evaluate(() => document.body.classList.remove('coach-kb'));
await page.waitForTimeout(200);
...
await page.waitForTimeout(600);
ok(!!(await page.$('#coachMem')), 'clearing the chat keeps the memory strip');
